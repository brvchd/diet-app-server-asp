...
namespace diet_server_api.Services.Implementation.Repository
{
    public class KnowledgeBaseRepository : IKnowledgeBaseRepository  
    {
        ...
        public async Task<UpdateDiseaseResponse> UpdateDisease(UpdateDiseaseRequest request)
        {
            ...
            disease.Name = request.Name == null || string.IsNullOrWhiteSpace(request.Name) ? disease.Name : request.Name;

            disease.Recomendation = request.Recomendation == null || string.IsNullOrWhiteSpace(request.Recomendation) ? disease.Recomendation : request.Recomendation;
            
            disease.Description = request.Description == null || string.IsNullOrWhiteSpace(request.Description) ? disease.Description : request.Description;
        ...
        public async Task<UpdateSupplementResponse> UpdateSupplement(UpdateSupplementRequest request)
        {
            ...
            supplement.Name = request.Name == null || string.IsNullOrWhiteSpace(request.Name) ? supplement.Name : request.Name;

            supplement.Description = request.Description == null || string.IsNullOrWhiteSpace(request.Description) ? supplement.Description : request.Description;
