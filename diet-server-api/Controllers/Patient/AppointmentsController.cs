...
namespace diet_server_api.Controllers.Patient
{
    ...
    public class AppointmentsController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAppointments()
        {
            ...
            try
            {
                var results = await _appointmentService.GetPatientAppointmentDates(nameIdentifier);
                return Ok(results);
            }
            catch (NotFound ex)
            {
        ...
        public async Task<IActionResult> GetAppointmentsByDate()
        {
            ...
            try
            {
                var results = await _appointmentService.GetPatientAppointmentsByDates(nameIdentifier);
                return Ok(results);
            }
            catch (NotFound ex)
            {
        ...
        public async Task<IActionResult> GetAppointmentsDetails([FromRoute] int idVisit)
        {
            try
            {
                var results = await _appointmentService.GetPatientAppointmentDetails(idVisit);
                return Ok(results);
            }
            catch (NotFound ex)
            {
